...
async function getQRCodeData() {
  ...
  console.log(qrcodeData, 'this is qr code')
 ...
 */
async function sendTo(phoneOrContact, message, artificialDelay) {
  ...
  try {
    ...
    await page.waitFor(artificialDelay); // artificial delay before sending out another message
 ...
 * @param {number} artificialDelay Delay in milliseconds after each message is sent
 * Send same message to every phone number
 */
async function send(phoneOrContacts, message, artificialDelay = 1000) {
  for (let phoneOrContact of phoneOrContacts) {
    await sendTo(phoneOrContact, message, artificialDelay);
  }
}
